...
payload contract, and hermetic NDK ABI fixture are implemented. A candidate remains
`releaseEligible=false` until the source-built static module graph, contrib link graph, notices,
and device playback evidence are all closed.
...
- close the exact static VLC module list and reject every GPL/unknown module;
- bind each linked contrib and toolchain archive to source, license, notice, and hash evidence;
- verify the real source-built `libvlc.so` DT_NEEDED/export surface for both ABIs;
